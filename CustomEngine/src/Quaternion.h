...
public:
	...
	explicit Quaternion(float xP, float yP, float zP, float wP);

	// Construct the quaternion from ana xis and angle. It is assumed that axis is already normalized
	...
	static Quaternion slerp(const Quaternion& a, const Quaternion& b, float f)
	{
		...
		{
			// Use linear interpolation if the quaternions are colinear
			scale0 = 1.0f - f;
			scale1 = f;
		}
